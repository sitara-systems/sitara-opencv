...
class exampleFaceTrackerApp : public App {
  public:
	struct FaceId {
		cv::Rect mRect;
		long id;
...
void exampleFaceTrackerApp::setup() {
...
void exampleFaceTrackerApp::mouseDown( MouseEvent event )
{
}

void exampleFaceTrackerApp::update() {
	cv::Mat frame;
	mCamera >> frame;
	detectFaces(frame);
...
void exampleFaceTrackerApp::draw() {
	gl::clear();
	gl::draw(mTexture);
}

void exampleFaceTrackerApp::detectFaces(cv::Mat& frame) {
	cv::Mat frame_gray;
	...
	for (size_t i = 0; i < mFaces.size(); i++) {
		...
		cv::putText(frame, std::to_string(mFaces[i].id), center + cv::Point(0, mFaces[i].mRect.height), cv::FONT_HERSHEY_PLAIN, 6, cv::Scalar(255));
	}
}

CINDER_APP( exampleFaceTrackerApp, RendererGl, [](App::Settings* settings) { settings->setConsoleWindowEnabled(); })
