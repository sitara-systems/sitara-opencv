...
void ContourFinder::findContours(cv::Mat img) {
	...
	if (useTargetColor) {
		...
		cv::Scalar base = toOcv(targetColor);
		if (trackingColorMode == TRACK_COLOR_RGB) {
			...
			cv::cvtColor(img, hsvBuffer, cv::COLOR_RGB2HSV);
			base = toOcv(sitara::opencv::convertColor(targetColor, cv::COLOR_RGB2HSV));
		...
		sitara::opencv::copyGray(img, thresh);
	}

	if (autoThreshold) {
		int thresholdType = invert ? cv::THRESH_BINARY_INV : cv::THRESH_BINARY;
		float maxVal = getMaxVal(thresh);
		cv::threshold(thresh, thresh, thresholdValue, maxVal, thresholdType);
	}
	...
	for (size_t i = 0; i < allIndices.size(); i++) {
		...
		ci::PolyLine2f p;
		for (auto point : contours.back()) {
			p.push_back(fromOcv(point));
		}
...
const std::vector<ci::PolyLine2f>& ContourFinder::getPolylines() const {
	return polylines;
}
...
ci::PolyLine2f& ContourFinder::getPolyline(unsigned int i) {
	return polylines[i];
}
...
cv::RotatedRect ContourFinder::getMinAreaRect(unsigned int i) const {
	return cv::minAreaRect(contours[i]);
}
...
void ContourFinder::setTargetColor(ci::Color targetColor, TrackingColorMode trackingColorMode) {
	useTargetColor = true;
...
void ContourFinder::setMinAreaRadius(float minAreaRadius) {
	minArea = M_PI * minAreaRadius * minAreaRadius;
	minAreaNorm = false;
}

void ContourFinder::setMaxAreaRadius(float maxAreaRadius) {
	maxArea = M_PI * maxAreaRadius * maxAreaRadius;
	maxAreaNorm = false;
}
